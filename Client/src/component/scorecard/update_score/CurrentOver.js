...
const CurrentOver = ({ ball }) => {
    ...
    return (
        <div
            className='mx-1 d-flex justify-content-center align-items-center position-relative current-over-item'
            style={{
                width: "45px",
                height: "45px",
                border: "4px double black",
                borderRadius: "50%",
                backgroundColor: typeof ball === 'string' && (ball.includes('noball') || ball.includes('Wicket')) ? 'red' : (typeof ball === 'string' && ball.includes('wide')) ? '#FEC007' : 'transparent',
                fontSize: "16px",
                position: "relative",
            }}
        >
