...
const friendslist = (req, res) => {
    const getlist = `
        ...
        WHERE u.id != ? AND u.id NOT in (SELECT sender_id from friend_request WHERE receiver_id = ? AND Status !=0 UNION SELECT receiver_id from friend_request WHERE sender_id = ? AND status !=0 ) `;
